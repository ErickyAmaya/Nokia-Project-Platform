...
function EditModal({ po, onClose, onSave }) {
  const [form, setForm] = useState({
    ...
    pci_description: po.pci_description || '',
  })
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <div className="fg" style={{ flex: 2 }}>
              ...
              <input className="fc" value={form.valor} onChange={e => setForm(f => ({ ...f, valor: e.target.value }))} placeholder="4180241" />
            </div>
            <div className="fg" style={{ flex: 1 }}>
              <label className="fl">Moneda</label>
              <select className="fc" value={form.moneda} onChange={e => setForm(f => ({ ...f, moneda: e.target.value }))}>
          ...
          <div className="fg">
            ...
            <input className="fc" value={form.supplier_name} onChange={e => setForm(f => ({ ...f, supplier_name: e.target.value }))} />
          </div>
          <div className="fg">
            <label className="fl">Condición de pago</label>
            <input className="fc" value={form.payment_terms} onChange={e => setForm(f => ({ ...f, payment_terms: e.target.value }))} />
          </div>
          <div className="fg">
            <label className="fl">Descripción (PCI)</label>
            <input className="fc" value={form.pci_description} onChange={e => setForm(f => ({ ...f, pci_description: e.target.value }))} />
          </div>
        </div>
  ...
  return (
    <>
      ...
      <div className="dash-hdr mb14">
        <div>
          <h1 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 22, fontWeight: 700, margin: 0 }}>POs</h1>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            disabled={uploading}
            style={{ background: '#144E4A', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: .5 }}
          >
            ...
            <tbody>
              {enriched.map(po => (
                <tr key={po.id} style={{ borderTop: '1px solid #f0f0f0' }}>
                  ...
                  <td style={{ padding: '7px 10px' }}>
                    <button onClick={() => setEditPO(po)} style={{ fontSize: 10, color: '#555', background: 'none', border: '1px solid #e0e4e0', borderRadius: 6, padding: '2px 8px', cursor: 'pointer' }}>
                      Editar
                    </button>
                  </td>
                </tr>
